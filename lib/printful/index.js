...
return _.isUndefined(text) ? text : text.trim();
  ...
  stringProductDataToObject(productStringData) {
    ...
    return {
      ...
      capsule: this.cleanText(arrayData[1]),
      name:this.cleanText(arrayData[2]) ,
      categorie: this.cleanText(arrayData[3]) ,
      sexe: this.cleanText(arrayData[4]) ,
    };
  }
...
const arrayData = productStringData.split('-');
    ...
    return {
      ...
      categorie: this.cleanText(arrayData[3]),
      sexe: this.cleanText(arrayData[4]),
      color:  this.cleanText(arrayData[5]),
      size: this.cleanText(arrayData[6]),
    };
  }
// product
  ...
  async getSyncProducts() {
    try {
      const { data } = await this.fetcher.get('sync/products');
      if (data.code === 200) {
        return data.result;
      }
      throw data.error;
