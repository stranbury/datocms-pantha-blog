...
const Fecther = require('./fetcher');
const { webhooksSchema, orderSchema } = require('../validator/printfulSchema');

// const color = require('color-name-list');
function Printful(apiKey) {
  this.base_url = 'https://api.printful.com/';
  this.fetcher = new Fetcher(this.base_url, Base64.encode(apiKey));
}
Printful.prototype.stringProductDataToObject = function (productStringData) {
...
Printful.prototype.stringVariantDataToObject = function (productStringData) {
  const arrayData = productStringData.split('-');
...
//     if(rawProducts){
//       const productsVariants = await Promise.all(rawProducts.map((raw)=>
//         this.getRawProductVariants(raw.id)
//       ))
//       const products = rawProducts.map((raw, index) =>{
//         const variants = productsVariants[index].map(rawVariants=>{
//           return {...rawVariants , ...this.stringVariantDataToObject(rawVariants.name)}
//         })
//         return {
//           ...raw,
...
//       const {data} = await this.fetcher.post(`orders/estimate-costs`, order);
//       if(data.code === 200){
//         return data.result
//       }
//     }
...
//     console.log("This error occured when create new order :", e, "with status :", order);
//     return new Error("can't get order -sys-error :");
...
//   }catch (error) {
//     const e = error.response.data ? error.response.data:  error ;
//     console.log("This error occured when try to getOrder:",  e, "with that data  :", orderId);
//     return new Error("can't get order -sys-error :");
...
//   try {
//     const {data} = await this.fetcher.delete(`orders/${orderId}`,{id:orderId});
//     if(data.code === 200){
//       return data.result
//     }
//   } catch (error) {
//     const e = error.response.data ? error.response.data:  error ;
//     console.log("This error occured when try to cancelOrder:",  e, "with that data  :", orderId);
//     throw new Error("syst error: check log")
//   }
// }
// async confirmOrder(orderId){
//   try {
//     const {data} = await this.fetcher.delete(`orders/${orderId}/confirm`,{id:orderId});
//     if(data.code === 200){
//       return data.result
//     }
//   } catch (error) {
//     const e = error.response.data ? error.response.data:  error ;
//     console.log("This error occured when try to confirmOrder:",  e, "with that data  :", orderId);
//     throw new Error("syst error: check log")
//   }
// }
// async getContryCode(){
