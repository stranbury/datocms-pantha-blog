...
export default class Printful {
  ...
  cleanText(text){
return _.isUndefined(text) ? text : text.trim();
  ...
  stringProductDataToObject(productStringData) {
    ...
    return {
      brand: this.cleanText(arrayData[0]),
      brandDetail: this.cleanText(arrayData[1]),
      capsule: this.cleanText(arrayData[2]),
      name:this.cleanText(arrayData[3]) ,
      categorie: this.cleanText(arrayData[4]) ,
      sexe: this.cleanText(arrayData[5]) ,
    };
  }
...
const arrayData = productStringData.split('-');
    ...
    return {
      ...
      categorie: this.cleanText(arrayData[4]),
      sexe: this.cleanText(arrayData[5]),
      color:  this.cleanText(arrayData[6]),//change to color Hex
      size: this.cleanText(arrayData[7]),
    };
  }
  ...
  async getShopProductsSearch(search) {
   ...
   async getShopProducts() {
    try {
      const rawProducts = await this.getSyncProducts();
      if (rawProducts) {
        const productsVariants = await Promise.all(
          rawProducts.map((raw) => this.getRawProductVariants(raw.id)),
        );
  ...
  //mockup 
  async createMockupGeneratorTask(id, taskData) {
    try {
      const joiValidation = taskDataSchema.validate(taskData); 

      if(joiValidation.error != undefined){
        const { data } = await this.fetcher.post(`mockup-generator/create-task/${id}`, taskData);
        if (data.code === 200) {
          return data.result;
        }
        throw data.error;
      }
      throw joiValidation.error;
    } catch (error) {
      console.log('This error occured :', error, 'with id :', id);
      return new Error("can't get product -sys- :", error);
    }
